...
<tr data-student-id="<%= obj.student_id %>">
    ...
    <td><%= obj.category %></td>
    <td><%= obj.email %></td>
    <td style="cursor: pointer" title="view all books" onclick="showBookIssued(<%= obj.student_id %>)">
        <%= obj.books_issued %></td>
    <td>
        <a class="btn btn-primary student-status" data-status="blocked" data-table="approved-table">Block</a>
    </td>
</tr>

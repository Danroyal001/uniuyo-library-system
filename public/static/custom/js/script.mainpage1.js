    ...
    $.ajax({
        ...
        success: function (data) {
            if ($.isEmptyObject(data)) {
                table.html('<tr><td colspan="99" style="text-align:center">No such books found in library</td></tr>');
            } else {
                table.html('');
...
$(document).ready(function () {
    ...
    $(".homepage-form-submit").click(function () {
        ...
        switch (mode) {
            case 'book':
                var search_query = form.find('#find_author_or_book').val();
                if (search_query != '')
